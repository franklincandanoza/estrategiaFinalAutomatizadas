...
const url2 = "http://localhost:2368/ghost/#/site";
...
const LOG_FILENAME = "../../../results/smart-monkey-execution_ghost.html";
...
function avPag(){
    ...
    if(curPageMaxY - curY >= viewportHeight){ 
        if(curPageMaxY - (curY + viewportHeight) >= viewportHeight){
            ...
            cy.scrollTo(curX, curY)
            //cambiar por cy.window().scrollTo('bottom', { ensureScrollable: false })
...
function fillLoginPage()
{
  ...
  cy.session([username, password], () => {
    ...
    cy.visit(url).then((win)=>{
                 ...
                 cy.log("Primera visita");
                let $elements = win.document.getElementsByTagName("input");
                if($elements.length > 0)
                 {
                    for(var i=0;i< $elements.length; i++)
                    {
                        var randomElement = $elements.item(i);
                        if(!Cypress.dom.isHidden(randomElement))
                        {
                            cy.log(`EL atributo seleccionado es  ${ randomElement.getAttribute("type")} y su id es${randomElement.id}`);
                            if(randomElement.getAttribute("type")==='email')
                                cy.wrap(randomElement).type('c.agudeloh@uniandes.edu.co',{force: true});
                            if(randomElement.getAttribute("type")==='password')
                                cy.wrap(randomElement).type('Ghos2022..',{force: true});

                        }
                        cy.wait(1000);
                    }
                 }
                 else{ cy.log("NO hay elementos input");}
                 let $elements2 = win.document.getElementsByTagName("button");
                if($elements2.length > 0)
                 {
                    for(var j=0;j< $elements2.length; j++)
                    {
                        var randomElement2 = $elements2.item(j);
                        if(!Cypress.dom.isHidden(randomElement2))
                        {
                            console.log(`EL atributo seleccionado es  ${ randomElement2.getAttribute("type")} y su id es${randomElement2.id}`);
                            if(randomElement2.getAttribute("type")==='submit')
                                cy.wrap(randomElement2).click({force: true});
